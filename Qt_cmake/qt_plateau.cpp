...
Qt_Plateau::Qt_Plateau(QWidget *parent) : QWidget(parent) {
  ...
  setFixedSize(
      371 * 0.75,
      446 *
          0.75); // Adjust size based on number of rows, columns, and jeton size
}
...
void Qt_Plateau::paintEvent(QPaintEvent *event) {
  ...
  QPixmap stretchedPixmap = backgroundPixmap.scaled(
      371 * 0.75, 446 * 0.75, Qt::IgnoreAspectRatio, Qt::SmoothTransformation);
...
void Qt_Plateau::connectJetons() {
  for (int i = 0; i < NJETONS; i++) {
    connect(this->jetons[i], &Qt_jeton::jetonClicked,
